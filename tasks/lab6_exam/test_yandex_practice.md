...
Данный функционал сервиса по дате проверяет загруженность пользователя и рекомендует 
пользователю запланировать встречу или перенести её. 

Необходимо определиться по какому пути пойдём в обработке маршрута. Есть варианты, например:

1. `event/2023-10-15`, тогда будет маршрут вида `event/<date>`
...
class DateView(View): 
    def get(self, request: HttpRequest, date: str) -> HttpResponse:   
...
urlpatterns = [
    path('event/<date>', DateView.as_view()),
]
```
...
2. `event/2023/10/15`, тогда будет маршрут вида `event/<int:year>/<int:month>/<int:day>`
...
class DateView(View): 
    def get(self, request: HttpRequest, year: int, month: int, day: int) -> HttpResponse:  
        try:
...
urlpatterns = [
    path('event/<int:year>/<int:month>/<int:day>', DateView.as_view()),
]
```
...
class DateView(View): 
    def get(self, request: HttpRequest, dt: datetime) -> HttpResponse:  
        ...
...
* Мы ознакомились с стандартными конвертерами в Django, такими как `str`, `slug`, `int`, `uuid`, `path` 
...
Практические задания выложены в репозиторий - https://github.com/aeksei/YandexCourseCreatorEduTools
