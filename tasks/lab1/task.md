...
* Подставить свой ключ для API
* Дописать реализацию значений ключей в переменной `result` (как написать реализацию, написано ниже)
  * `temp`;
  * `feels_like_temp`;
  * `pressure`;
  * `humidity`;
...
Полноценное рассмотрение элементов и атрибутов приведено в файле `html_css.md`, почитайте его для полноценного 
понимания тегов и каскадных стилей.

#### *Основные элементы HTML5*

Ниже приведено большинство тегов HTML для ознакомления с многообразием тегом. 
Далее рассмотрим основные и чаще применительные теги.

![29.png](pic_for_task/29.png)
